...
# [Race Number, Country, City, Latitude, Longitude, Months to Race]
races = [[sublist[0], sublist[1], sublist[2], sublist[10], sublist[11], sublist[12]] for sublist in racesInfo]
...
races_months = ["March", "March", "March", "April", "April", "April", "May", "May", "May", "June", "June", "June",
                "July", "July", "August", "September", "September", "September", "October", "October", "October",
                "November", "November", "December"]


def test_constraints(solution):
    if solution[0] != 0 or solution[23] != 23:
        return False

    for i in range(len(solution) - 1):
        if races[solution[i]][1] == races[solution[i + 1]][1]:
            return False

    for i in range(len(solution)):
        not_possible_months = [j.strip() for j in races[solution[i]][5].split(',')]
        if races_months[i] in not_possible_months:
            return False
    return True
...
def create_order():
    digits = list(range(1, 23))

    valid = False
    while not valid:
        order = random.sample(digits, len(digits))
        order.insert(0, 0)
        order.append(23)
        valid = test_constraints(order)
...
def main(mutation_rate):
    ...
    return cal_distance(population[0])
